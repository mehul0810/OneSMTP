...
- Candidate branch: `release/0.4.0`, live origin tip `3bd181213afe43c84989b8f620ab86d997b676c8` at the PR #172 reconciliation snapshot.
...
| Customer-owned Gmail and Zoho Mail OAuth lifecycle, token refresh/revocation UX, and Gmail API delivery are available behind `provider_auth_lifecycle`. | Candidate-shipped (unreleased) | `src/Providers/Auth/ProviderOAuthLifecycleCoordinator.php`, `src/Api/ProviderOAuthController.php`, `src/Providers/Adapters/GmailAdapter.php`, `docs/admin/provider-setup.md`, and `docs/developer/provider-auth-lifecycle.md`. Gate-off preserves core behavior and existing Zoho manual refresh credentials; no hosted broker, shared secret, telemetry, or release metadata is implied. | Synthetic PHPUnit fixtures cover state binding/replay, exact scopes/endpoints, Zoho PKCE, token exchange/refresh, Gmail MIME/message-ID normalization, disconnect cleanup, and redaction. Browser proof is fixture-only; customer app registration and Google verification remain operational prerequisites. |
| Any claim that tracking or production license activation/update delivery is available. | Planned/excluded | Backlog issue [#40](https://github.com/mehul0810/aculect-mail/issues/40); concrete commercial licensing and update delivery remain separately deferred beyond the repository-local #66 foundation. | No shipped source, accepted docs, or merged PR proves these production-service claims. |
...
It also captures the customer-owned OAuth drawer at desktop and 390px widths
under `docs/admin/screenshots/issue-50-51/`; the fixture uses synthetic Gmail
and Zoho records and provider status responses only.
